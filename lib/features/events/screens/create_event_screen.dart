...
class _CreateEventScreenState extends State<CreateEventScreen> {
  ...
  void _createEvent() async {
    ...
    String? imageUrl;
    if (_coverImage != null) {
      imageUrl = await supabaseEventService.uploadEventImage(_coverImage!);
    }
    ...
    final event = Event(
      ...
      // clubName: widget.clubName, // Not stored in event table directly usually, but model has it? 
      // Checking local model... Event model has clubName. 
      // But Supabase table might not have it or we join it.
      // For now passing it if model supports it, but Supabase insert will ignore if not in JSON.
      // SupabaseEventService._eventToJson maps club_id but not club_name? 
      // Let's check SupabaseEventService._eventToJson again. It maps club_id. 
      // So clubName won't be persisted unless we add it or join.
      // That's fine for now.
    ...
    // We can't set clubName effectively for the table insertion unless we add a column, 
    // but the service handles insertion.

    final success = await supabaseEventService.createEvent(event);
