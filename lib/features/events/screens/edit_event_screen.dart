...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  void _saveEvent() async {
    ...
    // We don't need to rebuild the object manually if we update the service, but constructing it here
    // ensures we send fresh data. Note: Images handling might be needed if edit supports it (it doesn't in this UI yet).
    final updatedEvent = Event(
      ...
      imageUrl: widget.event.imageUrl, // Preserve existing image
    );

    final success = await supabaseEventService.updateEvent(updatedEvent);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Failed to update event'),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final success = await supabaseEventService.deleteEvent(widget.event.id);
