...
class _EditClubScreenState extends State<EditClubScreen> {
  ...
  void initState() {
    ...
    _taglineController = TextEditingController(); // Club model doesn't have tagline field yet
    _category = widget.club.category;
    // TODO(feature): Add isPrivate field to Club model and read from widget.club.isPrivate
    _isPrivate = false;
  }
  ...
  Future<void> _saveChanges() async {
    ...
    if (_selectedIcon != null) {
      final url = await supabaseClubService.uploadImage(_selectedIcon!, 'logo_${widget.club.id}');
      if (url != null) newLogoUrl = url;
    }

    String? newCoverUrl = widget.club.coverImageUrl;
    if (_selectedCover != null) {
      final url = await supabaseClubService.uploadImage(_selectedCover!, 'cover_${widget.club.id}');
      if (url != null) newCoverUrl = url;
    }

    // Construct updates map for Supabase
    final updates = {
    ...
    final success = await supabaseClubService.updateClub(widget.club.id, updates);
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        Navigator.pop(context);
         ...
         ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(supabaseClubService.error ?? 'Failed to update club'),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final success = await supabaseClubService.deleteClub(widget.club.id);
              if (ctx.mounted) Navigator.pop(ctx); // Pop dialog
              
              if (success && context.mounted) {
                 Navigator.pop(context); // Pop edit screen
                 Navigator.pop(context); // Pop dashboard if needed? Check nav stack.
                 ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
