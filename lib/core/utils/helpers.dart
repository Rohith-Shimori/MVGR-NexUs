...
class Validators {
  ...
  /// Roll number validator for MVGR College (JNTUK format)
  /// Format: YY + College Code (3 digits) + Branch (1 letter) + Number (2 digits) + Section/Roll (1-2 alphanumeric)
  /// Examples: 24331A05N6, 23331A0501, 22331A1234
  static String? rollNumber(String? value) {
    if (value == null || value.isEmpty) {
      return 'Roll number is required';
    }
    // JNTUK/MVGR format: 2 digits year + 3 digits college + 1 letter branch + 2 digits + 1-2 alphanumeric
    // Also supports older format: 2 digits + 3 letters + 4 digits
    final jntukPattern = RegExp(r'^\d{5}[A-Z]\d{2}[A-Z0-9]{1,2}$', caseSensitive: false);
    final oldPattern = RegExp(r'^\d{2}[A-Z]{3}\d{4}$', caseSensitive: false);
    
    final upperValue = value.toUpperCase();
    if (!jntukPattern.hasMatch(upperValue) && !oldPattern.hasMatch(upperValue)) {
      return 'Please enter a valid roll number (e.g., 24331A05N6)';
    }
    return null;
  }
